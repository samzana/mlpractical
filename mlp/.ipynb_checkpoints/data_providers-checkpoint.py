...
class MNISTDataProvider(DataProvider):
    ...
    def to_one_of_k(self, int_targets):
        ...
        one_hot_encoded_matrix = np.zeros((num_data, 10))
        i = 0
        for digit in int_targets:
            one_hot_encoded_matrix[i, digit] = 1
            i+=1
...
class MetOfficeDataProvider(DataProvider):
    ...
    def __init__(self, window_size, batch_size=10, max_num_batches=-1,
        ...
        weather_data = np.loadtxt(data_path, skiprows=3)
        relevant_weather_data = weather_data[:, 2:]
        #TODO: filter out all missing datapoints and flatten to a vector
        filter_array = relevant_weather_data != -99.9
        filtered_data = relevant_weather_data[filter_array]
        ...
        arr = []
        while j < len(standardized_data):
            arr.append(standardized_data[i:j])
            i+=1
            j+=1

        windowed_data = np.array(arr)

        #TODO: separate into inputs and targets
        # inputs are the first (window_size - 1) entries in windows
        inputs_list = []
        for window in windowed_data:
            inputs_list.append(window[0: window_size - 1])
